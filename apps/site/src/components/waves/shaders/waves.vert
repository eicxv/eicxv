...
void getTranslation(float i, float sign, out vec2 translation) {
  if (i < 0.5) {
    translation = vec2(-sign * moveA, -sign * moveC);
  } else if (i < 1.5) {
    translation = vec2(sign * moveA, -sign * moveC);
  } else {
    translation = vec2(0., sign * moveB);
  }
}
...
float waves(vec2 coord, float time) {
  float val = noiseAmplitude * snoise(vec3(coord, noiseSpeed * time));
  val += waveAmplitude * sin((waveWaveLength * coord.y + waveSpeed * time) * wavePeriod);
  return val;
}
...
void main() {
  ...
  translatedPos = a_position.xy + translationVec;
  vec3 p0 = vec3(translatedPos.x, waves(translatedPos, t), translatedPos.y);

  i = mod(a_index + 1., 3.);
  getTranslation(i, sign, translationVec);
  translatedPos = a_position.xy + translationVec;
  vec3 p1 = vec3(translatedPos.x, waves(translatedPos, t), translatedPos.y);

  i = mod(a_index + 2., 3.);
  getTranslation(i, sign, translationVec);
  translatedPos = a_position.xy + translationVec;
  vec3 p2 = vec3(translatedPos.x, waves(translatedPos, t), translatedPos.y);
  //end loop
