...
vec3 adjustPosition(vec3 p, vec3 v, vec3 a0) {
    return p + 0.5 * v * u_dt + 0.125 * u_dt * u_dt * a0;
}
...
void main()	{
    ...
    position = adjustPosition(position, velocity, a0);
    vec3 acceleration;
    vec2 ref;
    ...
    for ( float s = 0.5; s < textureSize.x; s++ ) {
        for ( float t = 0.5; t < textureSize.y; t++ ) {
            ...
            vec3 other_a0 = texture2D( u_textureCoefficients0, v_textureCoord ).xyz;
            otherPos = adjustPosition(otherPos, otherVel, other_a0);
